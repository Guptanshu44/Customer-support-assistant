...
- **[Customers.jsx](file:///c:/Users/gupta/Desktop/Infy-vi/omniDesk-copilot/frontend/src/pages/Customers.jsx)**:
  - Excluded all legacy mock names (`Sarah Mitchell`, `Alex Morgan`, `Jessica Taylor`, `Liam Vance`, `Elena Rostova`, etc.) and mock ticket IDs from the customer index.
  - Only genuine, fresh customer accounts generated by real active interactions (such as `anu` and `Aniket`) are displayed.
  - Added a **Delete Customer Record** action in the customer details drawer allowing administrators to remove any customer profile and associated sessions on demand.
- **[firebase.js](file:///c:/Users/gupta/Desktop/Infy-vi/omniDesk-copilot/frontend/src/api/firebase.js)**:
  - Added `purgeMockFirestoreRecords()` which automatically scans and removes legacy mock conversation and ticket documents from Firestore collections (`conversations`, `carebot_tickets`, `carebot_sessions`).
  - Added `deleteCustomerByName(name)` to delete all Firestore interaction documents associated with a specified customer.
- **[Tickets.jsx](file:///c:/Users/gupta/Desktop/Infy-vi/omniDesk-copilot/frontend/src/pages/Tickets.jsx)** & **[client.js](file:///c:/Users/gupta/Desktop/Infy-vi/omniDesk-copilot/frontend/src/api/client.js)**:
  - Filtered cloud tickets and session stores against mock names and mock IDs.
- **[server/app.py](file:///c:/Users/gupta/Desktop/Infy-vi/omniDesk-copilot/server/app.py)**:
  - Removed `_create_initial_session()` fallback that seeded `Alex Morgan` / `TK-8492` into SQLite.

## Verification
- Built frontend production bundle: `npm run build` completed with code 0 (`dist/index.html` bundled in 2.83s).
- Staged, committed, and pushed all updates to `origin/main` (`https://github.com/Guptanshu44/Customer-support-assistant.git`).
- Confirmed only fresh, user-generated customer sessions are indexed on `/Customers`.
